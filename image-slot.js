...
(() => {
  ...
  const stylesheet =
    ...
    // --slot-frame-bg lets a page match the frame's fill to a letterboxed
    // image's own background so contain-fitted screenshots read as seamless.
  ...
  class ImageSlot extends HTMLElement {
    ...
    _applyView() {
      ...
      // With fit=cover, position:"… top|0%" anchors the untouched crop to the
      // frame's top edge instead of centring it. A user reframe (view.y != 0)
      // always wins.
      const pos = (this.getAttribute('position') || '').trim().toLowerCase();
      const anchorTop = /(^|\s)(top|0%|0px|0)$/.test(pos) || pos === 'top';
      const t = (anchorTop && this._view.y === 0)
        ? (g.ih * k / g.fh * 50) + '%'
        : (50 + this._view.y) + '%';
